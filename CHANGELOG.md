...
Mevcut sürüm: **v2.6.87**

Yapısal kilometre taşları (kronolojik tersine):

- **v2.6.87** — Faz 4 SON ADIM: Eski blob yazımı tamamen kapatıldı. `cloud.js`'de hem yazma hem okuma path'leri devre dışı: `queueSync`, `_doSync`, `pushNow` artık no-op (per-table cloud-pertable.flushNow yeterli emniyet sağlıyor); `pull()` artık sadece per-table'dan okuyor (blob promise null sonuca sabitlendi). Sebep: blob ve per-table'a çift yazma kod karmaşıklığını koruyordu, Supabase storage'ını boşa şişiriyordu, gereksiz hale gelmişti. Per-table güvenilirliği v2.6.84 (workspaces data column fix), v2.6.85 (pull-in-progress), v2.6.86 (demo cleanup) ile doğrulandı. `user_data` tablosu DB'de duruyor (manuel arşivleme veya DROP migration ayrı yapılabilir). Mevcut blob verisinin kullanıcının yedek dosyasına gerek yoktu — per-table veriler tam.
- **v2.6.86** — Demo onboarding seed'in cloud'a sızması durduruldu. Gerçek kök neden: `js/seed/demo-recipes.js` boot sırasında aktif workspace'in (bootstrap'in yarattığı boş "My Kitchen") içine recipe + ingredient + menu + event seediyor. Workspace artık "boş" olmadığı için `cloud.js:isEmptyGhostWs` filter'ı onu drop edemiyordu; v2.6.85'in pull-in-progress flag'i de bunu çözmüyordu (demo merge sırasında kalıyor, sonradan flush ediliyordu). Düzeltme `auth.js`'de SIGNED_IN handler'ında: pull başlamadan önce `(demoSeeded && !hasUser)` ise `store.clearUserData()` çağrılır.
