...
**Roster** — haftalık vardiya ızgarası (personel × gün); hücre = saat (start/end) veya 6 durum kodu renk-kodlu: OFF, AL (izin), PH (resmi tatil), SL (hastalık), RDO (dinlenme), UNP (ücretsiz). Departman grupları, işçilik maliyeti göster/gizle (Pro), ⚡ haftayı doldur, önceki haftayı kopyala (isimle eşleştirir), canlı önizleme (tek motor — print/Excel/JPEG aynı; mobilde tıkla→zoom). **İşçilik % KPI (Pro):** haftalık ciro + hedef % → işçilik/satış % renk-kodlu hedef bandı (yeşil/amber/kırmızı; çıktılarda maliyet açıkken); ciro tek tıkla **o haftanın onaylı/tamamlanmış event'lerinden** doldurulabilir (Events↔Roster bağı; gelir hesabı events.js aynası). Çıktıda **departman alt-toplamı** + **günlük toplam satırı** (hangi gün ağır görünür). Print A4 yatay renkli (kenar boşluğu 3mm) + Excel (yatay, tek sayfa) + JPEG paylaşım · metin boyutu XXS–L (5 kademe) · Sync ✓ (`rosters`).
