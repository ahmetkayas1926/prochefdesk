...
v2.43'te 10 ölü tool dosyası silindi (`app/js/tools/`'tan) — route'lanmıyor, hiçbir `<script>` ile yüklenmiyordu. **8'i hâlâ silik** (gerekirse git geçmişinden canlandırılır):

`menu_matrix.js` (menuMatrix) · `sales.js` · `whatif.js` · `yield.js` (yield_calc) · `team.js` · `allergens.js` (**NOT:** `core/allergens-db.js` CANLI, silinmedi) · `menus.js` (klasik; `'menus'` route'u `menu_studio.js` yükler) · `tools-hub.js`

**v2.44.21'de YENİDEN İNŞA edildi → artık CANLI araç (yeni kod):** `nutrition.js` · `variance.js` (`core/variance.js` motorunun UI'ı) · `waste.js` (eski şema-only → tam UI). Bu 3 + `portion` + `prep` v2.44.30'da sidebar yerine **sub-nav sekmelerinde** (Stok/Tarif/Liste grupları, `PCD.subNav`) — route'lar app.js'te DURUYOR.

**Hâlâ UI'sız sync tabloları (kod VAR, ekran YOK):** `shopping_lists` · `mise_plans` · `team` — cloud sync/realtime/backup şeması tam ama hiçbir araç bu state'i okumuyor/yazmıyor. Tüm route kayıtları `app/js/core/app.js`'tedir.
