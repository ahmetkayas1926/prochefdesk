...
**Mevcut sürüm:** **v2.8.99** (push'a hazır local; production v2.8.94).
...
| 48 | **Blog Faz B Round 1** (yeni 2 yazı, total 5 yazı yayında): (1) Buffet Food Cost Calculator: Hotel Banquet Math (~1850 kelime, target "buffet cost per cover" — 4-variable model + per-guest/pickup/refill/FC% target tabloları + worked example + Cornell auth link + Buffet Planner CTA). (2) Iftar Buffet Planning: Capacity, Cost, Waste Norms (~1750 kelime, target "iftar buffet planning hotel" — niş MENA pazar, 5-station kültürel yapı + MENA hotel baseline tablo + per-course refill + 5 kültürel kural + Hotel Management auth link + Iftar template CTA). Cross-link mevcut 3 yazıya. sitemap.xml + blog/index.html güncel (newest first). | v2.8.95 | ✅ |
| 49 | **Blog Faz B Round 2** (yeni 2 yazı, total 7 yazı yayında, topic-cluster SEO stratejisi): (1) Cook & Cool HACCP: 60→21→5°C in 6 Hours Explained (~1800 kelime, target "cook and cool HACCP" / "two-stage cooling rule" — termodinamik açıklama + 5 pratik fix + cooling log + 3 auth link: FDA + FSANZ + UK FSA + HACCP Cook & Cool form CTA). (2) Ingredient Yield Percentage: Hidden Cost in Raw Weight (~1700 kelime, target "ingredient yield percentage" / "EP vs AP cost" — 14-row yield tablosu + measurement methodology + cooking shrinkage chain + worked example $11.93 vs $4.80 + USDA auth link + Ingredient editor yield% CTA). HACCP yazısı + Food Cost yazısı = "hub", yeni 2 yazı = "spoke" — Google topical authority pattern. Cross-link parent yazılara güçlü. sitemap + blog/index güncel. | v2.8.96 | ✅ |
| 50 | **Blog Faz B Round 3** (yeni 2 yazı, total 9 yazı yayında, Kitchen Ops + Catering Production kategorileri): (1) Kitchen Prep List Templates: Mise en Place by Phase (~1750 kelime, target "kitchen prep list template" / "mise en place" — 5-faz model: stocks→sauces→protein→garnish→final + worked schedule 60-cover + morning briefing + 3 failure mode + Escoffier brigade authority link + Mise en Place tool CTA). (2) Recipe Scaling for Events: Portion Math Beyond Multiplication (~1700 kelime, target "scaling recipes for catering" — 7 non-linear pitfall + scaling formula + multi-recipe event aggregation + shopping list cascade 6-step + USDA food waste auth link + Portion Calculator + Events tool CTA). Cross-link strong. sitemap + blog/index güncel. | v2.8.97 | ✅ |
| 51 | **Blog Faz B Round 4** (yeni 2 yazı, total 11 yazı yayında): (1) Kitchen Cards vs Printed Recipes (~1650 kelime, target "kitchen card template" — reference vs operational doc ayrımı + layout kararları A4/A5 portrait/landscape + lamination + 5-col multi-batch + Wikipedia info design + Escoffier auth + Kitchen Cards tool CTA). (2) Restaurant Inventory Par Levels (~1700 kelime, target "restaurant par level" — par formula + safety buffer by ingredient class tablo + 3 usage measurement methods + weekly review 5-step + 5 common mistakes + US BLS auth + Inventory tool CTA). Cross-link to Yield % + Food Cost. sitemap + blog/index güncel. | v2.8.98 | ✅ |
| 52 | **Blog Faz B Round 5 SON ROUND** (yeni 2 yazı, total 13 yazı yayında ✅): (1) Food Waste Tracking: What 5% Waste Costs Your Business (~1700 kelime, target "food waste tracking restaurant" — 3-category model prep/service/plate + 6 reduction levers + worked example $16,200/yıl saved + WWF Hotel Kitchen auth + Waste tool CTA). (2) Allergen-Safe Menu Design: Legal Compliance for Chefs (~1750 kelime, target "allergen menu design" / "EU 14 allergens" / "Natasha's Law" — 4 gov regulatory framework + 4 failure mode + 7-protocol disclosure framework + 4 auth link: EU FIC + FDA + UK Natasha's + FSANZ + Allergen Guardrail/Menu Builder CTA). 🎯 Faz B 5-round plan tamam: total 13 yazı, topic-cluster grafiği, MENA niş (Iftar) + uluslararası coverage. | v2.8.99 | ✅ |
...
22. ~~i18n consistency tarama~~ — ✅ **v2.8.90'da yapıldı**. 6 dosyada 40+ hardcoded EN string → `t()` + 48 yeni i18n key. Round 3 tamam. Sonraki ihtiyaç çıkarsa Round 4 olur.
23. **Buffet Excel footer "Made with ProChefDesk · prochefdesk.com"** ⏳ — operatör low-priority not: Recipe Cost Excel'inde nazik footer var, buffet Excel'ine de eklenmeli. Tek hücre add operasyonu. 5 dakikalık iş, ayrı turda v2.8.92+ içinde paket halinde yapılabilir.
...
| Production sürümü | **v2.8.99** (push'a hazır local; production v2.8.94) |
