...
**Operatör:** Ahmet Kaya, Perth Western Australia, profesyonel chef. Solo non-commercial proje. Hedef: 6 ay içinde 20+ chef arkadaşını production kullanıcısı yapmak.

**Production sürümü:** **v2.8.0** (2026-05-06 deploy edildi)
...
- **Cache-busting (v2.8.0 sonrası):** `index.html`'de tüm `?v=__VERSION__` placeholder'ları, Cloudflare build command (`node build.js`) tarafından deploy zamanı `js/core/config.js`'teki `APP_VERSION` ile replace edilir. Yeni sürüm bump'ı **sadece config.js**'i değiştirir, başka hiçbir yere dokunulmaz.

### Frontend dosyaları (zip yapısı)
- `js/core/`: app.js, auth.js, cloud.js, cloud-pertable.js, cloud-realtime.js, config.js, i18n.js, idb-wrapper.js, photo-storage.js, qr.js, router.js, share.js, store.js, utils.js, variance.js, allergens-db.js
  - **`cloud-migrate-v4.js` v2.7.0'da silindi.** Eski blob → per-table migration script'iydi, görevi v2.6.92'de tamamlandı, hala dosya halinde duruyordu, v2.7.0'da tamamen silindi (auth.js'ten 2 çağrı + index.html'den script ref + dosyanın kendisi).
- `js/seed/`: demo recipes (misafir kullanıcıya seed)
- `js/tools/`: tool ekranları (recipes, ingredients, menus, events, suppliers, kitchen-cards, portion-calc, shopping-list, inventory, waste, checklists, haccp, account, dashboard)
- `js/ui/`: modal, toast, calendar, vs. UI primitives
- `js/i18n/`: en.js, tr.js, es.js, fr.js, de.js, ar.js + phase modülleri
- `build.js` (kök, v2.8.0'da eklendi): Cloudflare build hook için sürüm injection scripti
...
- `supabase/functions/backup-to-r2/index.ts` — **v3 (v2.8.0 öncesi yeniden yazıldı, 2026-05-06)**. v2 `user_data` tablosundan okuyordu (v2.6.87'de DROP edilmişti) → ~6 ay boş yedek üretmiş. v3 21 per-table tabloyu (16 ws-scoped + 5 top-level) ayrı ayrı `<YYYY-MM-DD>/<table>.jsonl` olarak R2'ye yazar + summary.json + photos-manifest.json. 30-day retention.
...
**v2.8.0 sonrası repo↔DB drift'i SIFIRDIR.** Disaster recovery'de DB sıfırdan kurulabilir, hiçbir trigger/function/cron kayıp olmaz.
...
## 4. Frontend ↔ DB Durumu (v2.8.0)

v2.8.0 itibarıyla **drift yok**. Repodaki migration'lar DB'deki gerçek durumu birebir yansıtıyor. Frontend'deki ölü kod (cloud-migrate-v4, user_data DELETE, blob merge branch) tamamen temizlendi (v2.7.0–v2.7.2).

## 5. Tamamlanmış İşler (Operatör Onaylı, v2.8.0'a Kadar)
...
| 16 | RLS audit (23 tabloda RLS aktif doğrulandı) | DB-only |
...
**1) App'i `/app/` altına taşı**

Şu an `prochefdesk.com/` doğrudan app'i açıyor. Lansman için `prochefdesk.com/` landing page göstermeli, app `prochefdesk.com/app/` üzerinden açılmalı. Riskli iş: OAuth callback URL'leri (Supabase Dashboard'da update gerekir), manifest.webmanifest path'leri, public_shares share link'leri, photo storage signed URL referansları etkilenir. Birden fazla sürümde yapılmalı: önce app dosyalarını /app/ klasörüne taşı, OAuth callback'i hem eski hem yeni URL'i kabul edecek şekilde ayarla, sonra manifest+routing güncelle, sonra eski URL'i 301 ile /app/'e redirect et, en son landing page'i root'a koy.

**2) Landing page**

`/app/` taşıması bittiğinde root'a marketing landing page koyulur. Şu an mevcut değil — yeni dosya yaratma işi. SEO için de kritik (chef'leri çekmek için).
...
**4) Realtime subscription leak Edge Case #2** — hızlı logout/login race. Lansman öncesi gerçekleşme ihtimali çok düşük (tek kullanıcı), atlandı.

### Gelecek / hazır altyapı (kullanılmıyor)

**5) Subscriptions tablosu** — Premium tier için DB'de hazır, kullanılmıyor. "Free now, premium possible later" planına göre.
...
### Yeni öğrenilen kurallar (2026-05-06 konuşmasından)
...
10. **Cloudflare Pages build hook ile sürüm injection (v2.8.0 sonrası):** `index.html`'de `?v=__VERSION__` placeholder'ları yaşar. Sürüm bump'ı **sadece** `js/core/config.js`'in `APP_VERSION` satırını değiştirir + CHANGELOG günceller. Cloudflare push'ta `node build.js` çalıştırarak placeholder'ları gerçek sürümle replace eder. **`index.html`'e bir daha asla manuel sürüm string'i yazma** — yoksa build fail eder.
...
| Production sürümü (frontend) | **v2.8.0** |
...
2. ☐ CHANGELOG.md son sürüm girdisi okundu mu? (Mevcut: v2.8.0)
...
5. ☐ DB durumunu varsayma — gerekiyorsa SQL ile sor
6. ☐ Cloudflare Pages durumunu varsayma — gerekiyorsa Dashboard ekran görüntüsü iste
7. ☐ Sürüm bump için sadece config.js'i değiştir, index.html'e dokunma (build hook v2.8.0'da kuruldu)
...
`PCD.icon(name, size)` fonksiyonu (`js/core/utils.js`'te) registry'de olmayan bir isim verilince **info ikonuna fallback** yapar. Hata fırlatmaz, console warn vermez — sadece kırmızı yuvarlak içinde "i" görüntüler.
...
**Mevcut registry'deki yaygın isimler:** `trash`, `refresh`, `archive`, `edit`, `plus`, `x`, `check`, `info`, `warn`, `chef-hat`, `chevron-down`, vb. (registry tam liste için utils.js'i incele.)

**Pratik kural:** Yeni ikon kullanmadan önce `grep -n "<name>:" js/core/utils.js` ile registry'de var olduğunu doğrula. Eğer yoksa ya o iconu registry'ye ekle, ya mevcut bir ikona düş.
...
- **Pull (cloud → local):** `cloud.js` boot'ta tüm 22 tablodan kullanıcıya ait satırları çeker, IDB'ye yazar, state'e merge eder. `pullInProgress` flag'i ghost duplicate önler (v2.6.85).
- **Realtime (cloud → local, canlı):** `cloud-realtime.js` 19 tabloya WebSocket subscribe olur. Postgres change event'leri local state'i otomatik günceller (cross-device sync). v2.7.9'da orphan channel leak fix'i eklendi.
...
**`/app/` taşıma sırasında bu URL'leri güncelleme adımı ATLAMA.** Eski URL listede tutulup yeni `/app/` URL'i de eklenmeli (geçiş süresi için), sonra eski temizlenir.
...
- **Misafir (login yok):** Sadece IDB, cloud yazma KAPALI. Demo seed yüklenir, onboarding tour gösterilir. Cloud kontaminasyonu önlemek için herhangi bir push tetiklenmemeli (v2.6.93'te bu sızıntı kapatıldı).
...
### 10.8 — index.html sürüm string'i ASLA literal yazılmaz (v2.8.0+)

Repodaki `index.html`'in 50 yerinde `?v=__VERSION__` veya `>v__VERSION__<` placeholder yaşar. Cloudflare deploy zamanı `node build.js` çalıştırarak bu placeholder'ları gerçek sürümle replace eder. Operatör veya AI tarafından `index.html`'e literal sürüm string'i yazılırsa (`?v=2.8.1` gibi):

- **Build başarısız olur:** `[build] FATAL: No __VERSION__ placeholders found in index.html`
- Cloudflare Pages deploy reddeder, eski sürüm canlı kalır (operatör için zarar yok ama yeni sürüm gitmez)

**Sürüm bump için tek dokunulan yer: `js/core/config.js`'in APP_VERSION satırı.**
