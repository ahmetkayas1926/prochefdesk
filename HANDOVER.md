...
Profesyonel şefler için offline-öncelikli web tabanlı mutfak yönetim sistemi: tarif maliyetlendirme, menü tasarımı, vardiya planı, HACCP uyumu ve mutfak operasyonu tek platformda. Perth WA'da aktif şef Ahmet Kaya tarafından inşa edilmekte + kullanılmaktadır. **Production:** `prochefdesk.com` (uygulama `/app/`).
...
- **Stripe:** Pro Monthly 19 AUD / Annual 190 AUD. 3 Edge Function (`create-checkout-session`, `create-portal-session`, `stripe-webhook` — imza doğrulamalı, plan'ı yazan tek otorite). Şu an **sandbox/test**; canlı = operatör açık adımları. **Payout:** TR doğrudan Stripe payout desteklemiyor → canlıya alma yolu **Stripe Atlas** (ABD LLC + EIN + Mercury); uygulama hazır, yalnız test→live + webhook kalır. **Durum (2026-06-17):** Atlas başvuruldu + ödendi, `ProChefDesk, LLC` (Delaware) kuruluyor — incorporation ~1-2 gün, EIN ~3-5 hafta (SSN yok). Incorporation sonrası operatör Mercury + "start accepting payments" yapar → Claude legal docs + test→live bağlar. Yıllık zorunlu (kaçırma): Delaware franchise ~$300 + federal 5472/1120. **Gelecek:** operatör AU PR alınca doğrudan **AU Stripe**'a geçilir (ABN + AU banka, LLC gereksiz), abonelikler yeni hesaba taşınır (kart/müşteri migration + key/webhook), ABD LLC feshedilir.
