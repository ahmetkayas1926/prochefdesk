...
**Watermark = `showWatermark()`.** `PCD.print` footer'ı (utils.js) + paylaşılan sayfa footer'ı (share.js) bu gate'ten geçer; Free'de kalır, Pro'da kalkar. Paylaşılan sayfada karar PAYLAŞANIN planına göre snapshot'a gömülür (`payload._wm`, create/refresh'te güncellenir) — görüntüleyeni etkilemez.

**Cost-view paylaşım modu.** `createOrGetShareUrl(kind, id, 'cost')` cost-share üretir (yalnız Pro). Maliyet YALNIZCA cost-share payload'una gömülür (`payload.cost` + `payload._mode='cost'`); normal public link sızdırmaz. `public_shares` tekilliği `(owner, kind, source, share_mode)` → bir kaynağın hem public hem cost linki olabilir. Görüntüleme `?view=cost` / `payload._mode` ile maliyet panelini render eder.
...
**Ortak styled-Excel = `PCD.xlsx`.** Yeni export'lar `PCD.loadXLSX().then(XLSX => PCD.xlsx.save(...))` (kalın yeşil başlık + çerçeve + alt-satır gölgesi + autofit). Roster kendi inline worksheet'ini kullanır (hücre rengi `PCD.xlsx`'te yok) — dokunma.
