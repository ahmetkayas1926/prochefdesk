...
**Roster aracı (v2.15.1 + v2.15.4).** `roster.js` (lazy, Kitchen). Çıktı motoru tek: **`rosterMatrix(data)`** → `{days, groups:[{name, rows:[{staff,cells:[{text,status?}],hours,cost}]}]}`; print/Excel/share hepsi bundan üretilir (tutarlı). **Departman grupları `groupedStaff(data)` — DİKKAT: `.group` döndürür, `.name` DEĞİL** (gridHtml'de `.group` oku; rosterMatrix `name:g.group`'a map'ler — v2.15.4'te bu uyumsuzluk bug'dı). Hücre = `{start,end,note}` (vardiya) VEYA `{status:'OFF'}` (izin/durum, 0 saat). Durum kodları `ROSTER_STATUS` registry (OFF/AL/PH/SL/RDO/UNP + renk; yeni kod → registry + `roster_st_*` i18n). Renkli print `print-color-adjust:exact` ile basar (Background graphics kapalı olsa bile — kaldırma). `data.venue` = print üst başlık bandı. Roster `data` jsonb içinde sync olur — yeni alan için migration gerekmez.
