...
**Mevcut sürüm:** v2.15.3 (kod hazır; operatör push etmeyi bekletiyor — 2.14.7→2.15.3 tek bundle). **Bu oturumda (v2.14.7 → v2.15.3):** **Roster (vardiya planı) aracı YENİ** (haftalık personel×gün ızgara + şablon/serbest saat + işçilik maliyeti **göster/gizle** toggle + history/print/Excel/paylaş + **bulut sync** v2.15.3) + **para birimi kalıcılık fix** (kaydet→yenile USD'ye dönüyordu; flush + cloud push eksikti) + tüm export/rapor/paylaşım `PCD.currencySymbol()`'e bağlandı + **menü allergen-safe baskı filtresi** manuel-kod tabanlı (18 chip) + **aktif filtre tüm öğeleri gizlerse uyarı** (manuel yemek görünmüyor bug'ı) + **tedarikçi sipariş geçmişi** (History: tarih/mesaj/alıcı/kanal). (Önceki v2.13.7-2.14.6: menü manuel harf kodları + fiyat stili, whiteboard mobil fix, tek Excel standardı, demo seed, Discover hesap-bazlı sayaç.) Detay: `CHANGELOG.md`.
...
- **v2.14.7 — v2.15.3:** **Para birimi kalıcılık fix** (kaydet→yenile USD'ye dönüyordu; account.js flush + user_prefs cloud push eksikti) + tüm export/rapor/paylaşım `PCD.currencySymbol()` (Excel numFmt + share menü fiyatı) + **menü allergen-safe baskı filtresi** manuel-kod tabanlı 18 chip + **tedarikçi sipariş geçmişi** (History butonu, son-50) + **Roster (vardiya) aracı YENİ** (`roster.js` lazy, Kitchen; personel×gün ızgara + şablon/serbest saat + işçilik maliyeti göster/gizle + history/print/Excel/paylaş) + **menü filtre tüm öğeleri gizlerse uyarı + temizle** + **Roster bulut sync** (`rosters` MAP-tablo, stock_count_history pattern, 6 dosya + migration; cloud.js HIGH_EDIT per-record merge). Tüm app cloud-sync kapsam denetimi → roster TEK boşluktu.
...
**Menü diyet/alerjen harf kodları (v2.14.1).** `menus.js` `MENU_CODES` registry: küçük harf = diyet/uygunluk (v/vg/gf/gfo/df/dfo/nf/h), BÜYÜK = "içerir" alerjen (N/G/D/E/F/SF/S/SE). Her menü öğesinin `it.codes` dizisi (id'ler) — **manuel**, otomatik tahmin YOK (eski EU-numara sistemi + emoji kaldırıldı). `menu.allergenStyle` 'codes'|'off'. Render: yemek adı yanında `(gf) (gfo)`, altta sadece kullanılan kodları açıklayan lejant. Editörde `PCD.picker` multi-select. Fiyat: `menu.priceStyle` 'symbol'|'plain'(simgesiz, Cornell)|'hidden' (eski `hidePrices`→'hidden'). Yeni kod eklersen MENU_CODES + 6 dilde `menu_code_*` i18n.
