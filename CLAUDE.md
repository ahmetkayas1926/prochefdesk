...
**Mevcut sürüm: v2.8.94** (push'a hazır local; production v2.8.93 — push edilene kadar). Detay: `CHANGELOG.md`.

**Master roadmap (operatör onayıyla):** ✅ v2.8.83-93 (Welcome tour + Profile+Discover + Excel bug + Buffet UX overhaul + 7 preset + i18n Round 3 + Dashboard + Portion Calc + 4 yeni Buffet preset) → ✅ v2.8.94 Blog SEO upgrade Faz A (mevcut 3 yazı: JSON-LD + authority + cross-link) → v2.8.95+ Blog Faz B (yeni 10 yazı aşamalı 1-2/sürüm) → v2.8.95+ paralel: yield + waste + variance + kalan NAKED araçlar (nutrition/allergens/mise/discover/account/team/sales/whatif/menu_matrix/haccp hub) → ~v2.9.x'e kadar tüm araçlar RICH seviyesinde + 10 yeni blog yazısı yayında. **Backlog:** Buffet Excel footer "Made with ProChefDesk · prochefdesk.com" — düşük öncelik, operatör "bir ara yaparsın" dedi.
