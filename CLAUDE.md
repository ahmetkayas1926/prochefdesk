...
**Mevcut sürüm: v2.8.99** (push'a hazır local; production v2.8.94 — push edilene kadar). Detay: `CHANGELOG.md`.

**Master roadmap (operatör onayıyla):** ✅ v2.8.83-93 (Welcome tour + Profile/Discover + Excel bug + Buffet UX overhaul + i18n Round 3 + Dashboard + Portion Calc + Buffet preset overhaul) → ✅ v2.8.94 Blog Faz A (3 SEO upgrade) → ✅ v2.8.95-99 Blog Faz B 5-round (10 yeni yazı: Buffet Cost + Iftar / Cook & Cool + Yield % / Mise en Place + Recipe Scaling / Kitchen Cards + Par Levels / Waste Tracking + Allergen Menu) **Total 13 yazı yayında** 🎯 → **v2.9.x master roadmap NAKED araç sweep:** yield + waste + variance + nutrition + allergens + mise + discover + account + team + sales + whatif + menu_matrix + haccp hub. Her tur 2-3 araç paketi. **Backlog:** Buffet Excel footer "Made with ProChefDesk · prochefdesk.com" — düşük öncelik.

**Blog SEO standardı (v2.8.94'te kurulan):** Her yeni blog yazısı şunları içermek zorunda — (1) `<head>` içinde JSON-LD Article schema, (2) body içinde en az 1 authority outbound link (gov/akademik, `target="_blank" rel="noopener"`), (3) footer'dan önce `<section class="related-posts">` 2-card cross-link, (4) sitemap.xml'e yeni `<url>` blok + tüm etkilenen `<lastmod>` güncel. Her yeni yazı push sonrası operatör manuel: Google Search Console → URL Inspection → "Request Indexing".

**Blog SEO standardı (v2.8.94'te kurulan):** Her yeni blog yazısı şunları içermek zorunda — (1) `<head>` içinde JSON-LD Article schema (`headline` + `description` + `datePublished` + `dateModified` + `author.Person` + `publisher.Organization` + `mainEntityOfPage` + `wordCount` + `keywords`), (2) body içinde en az 1 authority outbound link (USDA / FDA / FSANZ / Cornell / akademik paper — `target="_blank" rel="noopener"`), (3) footer'dan önce `<section class="related-posts">` 2-card cross-link (eski yazılara), (4) sitemap.xml'e yeni `<url>` blok + tüm etkilenen `<lastmod>` 2026-MM-DD güncel. Her yeni yazı push sonrası operatör manuel: Google Search Console → URL Inspection → yazının absolute URL'ini gir → "Request Indexing" (1-2 günde indekslenir, otomatik bekleme 2-3 hafta). Her tur tek bir sürüm bump. Baseline her araçta: (1) kapatılabilir inline guide (buffet v2.8.77 pattern), (2) her input altında italik gri 11-12px hint, (3) yarı-transparan örnek placeholder ("e.g. 800"), (4) empty state onboarding kartı. i18n: TR + EN tam parity, ES/FR/DE/AR EN fallback (operatör onayladı).
