...
**Mevcut sürüm:** v2.13.6 (production). **Bu oturumda (v2.12.0 → v2.13.6):** Checklist baştan yazıldı (2 tür: Control + Prep), profil persist fix (auth merge + flush), Privacy/Terms 6-dil, **Whiteboard WYSIWYG** (canvas = print birebir, tek render motoru), **Waste Log + Shopping List araçları KALDIRILDI** (Mise pattern: UI sil + cloud şema koru), Portion'a birleştirilmiş malzeme görünümü (Shopping consolidation taşındı), 6 yeni whiteboard template + print footer tek-sayfa fix, "Sync now" butonu kaldırıldı, Ctrl+wheel zoom hata spam fix. (Önceki: Mise en Place Planner v2.11.16'da kaldırılmıştı — schema korundu, Diet flags v2.10.3 pattern'ı.) Detay: `CHANGELOG.md`.
...
- Küçük: Privacy/Terms es/fr/de/ar native göz (çeviriler native değil); 3 yeni whiteboard template adı (Doneness / Prep Timeline / Mise) de/es/fr/ar'da EN fallback'te
