...
- [ ] Stripe canlı bağlantısı — **payout yolu: Stripe Atlas** (ABD LLC + EIN + Mercury banka + Stripe; TR'den pasaportla, ~1 hafta, ~500$ tek sefer + ~400-600$/yıl sabit). TR doğrudan Stripe payout DESTEKLEMİYOR → ABD LLC şart. Uygulama zaten Stripe'a hazır; Atlas bitince yalnız test→live anahtar + webhook canlı (kod tarafı Claude'da). Alternatif (gelir belirsizse, şirketsiz başlangıç): Lemon Squeezy MoR (~%5, ödeme katmanı yeniden bağlanır).
...
- [ ] Launch QA — beraber, doğrulama pass'leri. Kod/preview denetimi yapıldı: 18 rota mobil(375)+tablet(768) yatay taşma YOK · console hata 0 · network fail 0 · i18n simetrik (en=tr 3119, es/fr/de/ar 2816) tanımsız anahtar 0. Açık bulgular: Arapça RTL kısmi (orta — `data-dir` set ama gerçek `dir` yok, ~12 RTL kuralı) · `book` ikonu recipes "bulk to menu" generik info'ya düşüyor (düşük, `book-open` olmalı)
