...
**Mevcut sürüm: v2.8.91** (push'a hazır local; production v2.8.89 — push edilene kadar). Detay: `CHANGELOG.md`.

**Master roadmap (operatör onayıyla):** ✅ v2.8.83 Welcome tour → ✅ v2.8.84 Author profile-priority → ✅ v2.8.85 Profile↔Discover bağlantı → ✅ v2.8.86 Excel bug fix + Buffet list Cost Report parite → ✅ v2.8.87 Excel menu-item scope fix → ✅ v2.8.88 Buffet UX modernize Faz 1 → ✅ v2.8.89 Buffet Quick start preset → ✅ v2.8.90 i18n consistency Round 3 → ✅ v2.8.91 Dashboard + Tools-hub UX upgrade → v2.8.92+ NAKED araçlar (yield/waste/variance/nutrition/allergens/mise/discover/account/team/sales/whatif/menu_matrix/haccp hub, ~6-7 sürüm) → v2.8.99'a kadar tüm araçlar RICH seviyesinde. **Backlog:** Buffet Excel footer "Made with ProChefDesk · prochefdesk.com" — düşük öncelik, operatör "bir ara yaparsın" dedi. Her tur tek bir sürüm bump. Baseline her araçta: (1) kapatılabilir inline guide (buffet v2.8.77 pattern), (2) her input altında italik gri 11-12px hint, (3) yarı-transparan örnek placeholder ("e.g. 800"), (4) empty state onboarding kartı. i18n: TR + EN tam parity, ES/FR/DE/AR EN fallback (operatör onayladı).
