...
- [ ] **Temizlik — `core/variance.js` ölü dosya (2026-06-24 denetimi).** Dosya `index.html`'de YÜKLENMİYOR (grep: 0) + hiç yazılmayan `salesLog` tablosunu okuyor. Çalışan Variance aracı = `tools/variance.js` (kendi motoru, salesLog'a bakmaz). Ölü dosya silinebilir — davranış değişmez. Düşük öncelik, opsiyonel.
