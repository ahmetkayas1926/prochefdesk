...
- **Tahmin yürütme.** Değiştirmeden önce gerçek dosyayı oku; sorun gelince önce kod ile mevcut durumu kontrol et (mümkünse statik okuma ile), gerekirse DevTools console'a bak, sonra çöz.
- **Önizleme/tarayıcı kontrolü SADECE gerektiğinde, en fazla bir kez.** Görsel/render mantığını (layout, print, canvas, i18n string, scale-to-fit vb.) gerçekten etkileyen bir değişiklik yaptıysan önizlemeyi **aç → kontrol et → kapat**, tek seferde. "Emin olmak için" tekrar tekrar açma, spekülatif gezinme yapma, ilgisiz ekranları dolaşma yok. Metin/mantık/backend değişikliklerinde (görsel çıktı etkilenmiyorsa) önizleme açmaya gerek YOK. Dosyanın geri kalanındaki **"Doğrulama:"** notları operatöre bilgi amaçlı referanslardır — her ilgili değişiklikte otomatik/zorunlu olarak birebir icra edilecek adımlar değildir; yalnızca o alanda gerçek bir bozulma şüphesi varsa kullan.
...
**Edge Function'lar (9) elle deploy edilir.** Supabase dashboard'dan: `create-checkout-session`, `create-portal-session` (verify_jwt AÇIK — frontend user JWT yollar), `stripe-webhook` (verify_jwt KAPALI — Stripe imzayla doğrular, plan'ı yazan TEK otorite), `send-proposal-email` + `submit-event-signature` (etkinlik teklif/imza akışı, **Resend** ile e-posta), `rate-limited-view` (Discover görüntülenme), `delete-account`, `backup-to-r2` + `cleanup-photos` (cron). Kaynak `supabase/functions/`. **Kod değişirse Edge elle yeniden deploy — git push yalnız frontend'i günceller.** Stripe CANLI (secret'lar live).
...
**Landing'de HAZIR ama GİZLİ: şef referansları (testimonial) bölümü.** `index.html` → `<section id="testimonials" style="display:none;">` (canlıda görünmez — **boş alan yok, sahte yorum yok**) — 3 placeholder kart + kod-içi TR talimat. **Aktive:** `style="display:none;"` → `style=""` + kartlardaki `[İsim]`/`[Rol]`/`[alıntı]`'yı gerçek referansla doldur (somut sonuç: "teklif 2 saatten 15 dk'ya düştü"). Gerçek 1-3 referans gelmeden AÇMA. Canlı olanlar: **ROI bloğu** (pricing altı, ~$179→$19 karşılaştırması, 6 dil, `roi_*` anahtarları) + **kurucu bloğu** (`founder_*`, Marriott/Katar). **Hero** (H1 "Know the true cost of every plate" + eyebrow "Food costing for restaurants, cafés & caterers", 6 dil) ile **uygulama/marka adı "Food Business Management System"** (app `<title>`/splash/manifest/tour + landing `<title>`, 6 dil) AYRI kararlar — hero'ya dokunma. **Proof bölümü tek kesintisiz 8-kartlı grid** (Events/Buffet/Menu/Roster/HACCP/Inventory) + Client-proposal/HACCP-Audit-Pack belge blokları ardışık; Costs+Live tek "Always current" bölümünde, Intro+Why tek "Why ProChefDesk" bölümünde — ayrı ayrı tekrar YOK. Client-quote bloğu artık `.sigfeat` özel SVG/CSS grafiği (ekran görüntüsü değil, `evq_sat1/2/3`+`evq_card_tag` i18n, 6 dil).

**Landing `<title>` İKİ KAYNAKLI.** Statik `<title>` yalnız ilk boyama (FOUC) içindir — sayfa yüklenince `apply(lang)` scripti `document.title`'ı `page_title` sözlüğünden (`I18N.en` fallback + 6 dilin kendi `page_title`'ı) EZER. Başlığı değiştirirken ikisini de güncelle, yoksa değişiklik JS çalışınca sessizce eski metne döner.

**Event/Buffet: Liste→Preview→Edit (recipes.js deseni).** Liste tıklaması artık salt-okunur Preview'a açılır (kayıtlı veri, düzenlenemez); editör yalnız Save/Cancel/Delete taşır. TÜM çıktı (BEO/Proposal/Print/Excel/Share/Prep List/Order List/Cost Report/Duplicate) Preview'dadır, editörde YOK — eskiden editördeki çıktı butonları kaydedilmemiş taslağı sessizce upsert ediyordu. Yeni çıktı butonu editöre değil Preview'a eklenir. Event'te ayrıca imza tek mekanizma: public `?share=` linki (yüz yüze de aynı link açılır), editörde ayrı imza paneli yok.
...
**Photo storage flow + race.** Recipe foto → WebP @0.82 → Supabase Storage → public URL → `data.photo`. Upload async; save click anında `data.photo` eski olabilir → sync foto'suz gider. Foto görünmüyor raporunda (teşhis referansı, tek seferlik doğrulama yeterli): recipe'i aç → Save → 5sn → Discover Refresh.
