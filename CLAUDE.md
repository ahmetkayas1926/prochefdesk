...
- **v2.15.6 — Tek tablo kaynağı `buildRosterTable(data, showCost)`** (inline-stilli HTML): print + JPEG önizleme/gönderme + preview view AYNI bunu kullanır. Yazı boyutu `data.fontSize` (s/m/l, `FONT_SIZES`) + `data.bold`; `table-layout:fixed`+% genişlik → A4 yatay tek sayfa.
- **v2.15.6 — Excel OTOMATİK YATAY = JSZip enjeksiyonu (`saveXlsxLandscape`).** SheetJS/xlsx-js-style yazımda `<pageSetup>`/`<sheetPr>` ÜRETMEZ (round-trip ile doğrulandı) → Excel portre + çok sayfa açıyordu. Çözüm: XLSX.write→array → JSZip ile `xl/worksheets/sheet1.xml`'e `<sheetPr><pageSetUpPr fitToPage="1"/></sheetPr>` (worksheet açılışından sonra) + `<pageSetup orientation="landscape" fitToWidth="1" fitToHeight="0"/>` (`<pageMargins/>`'den sonra) enjekte → blob indir. JSZip CDN lazy (`loadJSZip`). XML element SIRASI önemli (sheetPr ilk çocuk; pageSetup pageMargins'ten sonra ignoredErrors'tan önce) — bozma. Başarısızsa düz `XLSX.writeFile`'a düşer. `ws['!pageSetup']` SheetJS'te işe yaramaz (sadece JSZip yolu çalışır).
- **v2.15.6 — JPEG gönder (`sendRosterImage`):** html2canvas CDN lazy (`loadHtml2Canvas`) → offscreen `buildRosterTable` → canvas → JPEG blob → `navigator.canShare({files})` ise `navigator.share` (mobil WhatsApp), değilse indir. Roster artık metin paylaşmaz (operatör: "roster sipariş listesi değil").
- **v2.15.6 — Geri tuşu:** editör/önizleme route-state (`render(view,params)` editId/previewId okur; arguments.length>=2 ise moddan, yoksa içeriden refresh korunur). Aç → `PCD.router.go('roster',{editId|previewId})`; Back/in-app back → `history.back()`. Liste satır tıkla = önizleme, Edit butonu = editör, duplicate = sonraki haftaya kopya.
